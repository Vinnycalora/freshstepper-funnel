    ...
    try {
        switch (event.type) {
            case "checkout.session.completed": {
                ...
                // ✅ 1) Persist payment update FIRST (and await it)
                const savedOrder = await upsertOrder({
                    id: session.id,
                    // keep createdAt stable inside upsert merge logic
                    customerEmail: email,
                    email,
                ...
                // ✅ 2) Sendcloud is “best effort” (don’t break Stripe webhook if it fails)
                ...
                if (delivery === "postal") {
                    try {
                        ...
                        await upsertOrder(next as any);

                        console.log("✅ Sendcloud parcel created:", parcel?.id);
                    } catch (e: unknown) {
                        const msg = e instanceof Error ? e.message : String(e);
                        console.error("⚠️ Sendcloud step failed (payment still saved):", msg);
                    }
                }
        ...
        // ✅ If the payment update part throws, return 500 so Stripe retries
        const msg = err instanceof Error ? err.message : String(err);
